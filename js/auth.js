...
const AUTH = (function(){
  ...
  function setOverride(email, pwd){ 
    const m = getOverrides(); 
    m[email] = pwd; 
    localStorage.setItem(KEYS.OVERRIDES, JSON.stringify(m)); 
  }
  ...
  function pushReport(r){ 
    const arr = JSON.parse(localStorage.getItem(KEYS.REPORTS) || '[]'); 
    arr.unshift(r); 
    localStorage.setItem(KEYS.REPORTS, JSON.stringify(arr.slice(0,500))); 
  }
  ...
  function wireLogin(){
    ...
    form.addEventListener('submit', async (ev)=>{
      ...
      if(user && expected && expected === pwd){
        ...
        if(err) err.textContent = 'Invalid email or password (demo).';
      }
    });
  }
  ...
  function changePassword(){
    ...
    const np = prompt('Enter new password (stored locally for demo only, min 4 chars):');
  ...
  function renderReportsSummary(){
    ...
    el.innerHTML = r.length ? r.map(rep=>`
      <div style="padding:8px;border-bottom:1px dashed rgba(255,255,255,0.02)">
        <strong>${rep.email}</strong> 
        <div style="font-size:13px;color:var(--muted)">${rep.quizId} — ${rep.score}% — ${new Date(rep.ts).toLocaleString()}</div>
      </div>`).join('') : '<div class="muted">No reports</div>';
  }
  ...
  function renderReports(){
    ...
    el.innerHTML = arr.length ? arr.map(rep=>`
      <div style="padding:8px;border-bottom:1px dashed rgba(255,255,255,0.02)">
        <div style="display:flex;justify-content:space-between">
          <strong>${rep.email}</strong><small class="muted">${new Date(rep.ts).toLocaleString()}</small>
        </div>
        <div style="font-size:13px;color:var(--muted)">${rep.quizId} — attempt ${rep.attempt} — ${rep.score}%</div>
      </div>`).join('') : '<div class="muted">No reports</div>';
  }

  return {
    fetchUsers, setSession, getSession, clearSession, wireLogin,
    logout, ensureScores, getScores, setScores, pushReport, getReports,
